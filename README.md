...
映射表格式和填写示例见 [`AGENTS.md`](AGENTS.md)。这张表是给**使用本包的项目**用的。pstack-pi 自己的开发不受它约束：这个仓库不需要 Herdr，不需要任何特定 subagent 扩展，也不需要 MCP。
...
- 每个技能文件里的相对链接都能解析到真实文件（没有未打包的引用）。
