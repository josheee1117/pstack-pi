...
Create note lets a user save a titled note from the browser or the CLI, cancel an unfinished draft, and confirm the saved note from a second user-facing view.
...
- Remove `Release checklist` and `CLI note` during fixture cleanup, but keep their proof artifacts.
