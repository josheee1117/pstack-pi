...
- **Shared state.** If two actors might both write, ask what happens. If the answer is not "nothing", default to per-actor state with a merge at the read boundary ([separate-before-serializing-shared-state](../../pstack/references/principles.md#separate-before-serializing-shared-state)).
...
- **Encode invariants in types.** Hard-to-misuse types beat runtime checks, which beat prose comments ([encode-lessons-in-structure](../../pstack/references/principles.md#encode-lessons-in-structure)).
- **Validate at boundaries, trust types inside** ([boundary-discipline](../../pstack/references/principles.md#boundary-discipline)). Business logic as pure functions. The shell stays thin.
- **Single source of truth per invariant.** Derive instead of sync.
- **Idempotent state transitions where applicable** ([make-operations-idempotent](../../pstack/references/principles.md#make-operations-idempotent)). Ask what happens if the operation runs twice or crashes halfway.
- **Short call chains.** If tracing the flow needs more than three files, flatten the hierarchy ([laziness-protocol](../../pstack/references/principles.md#laziness-protocol), [minimize-reader-load](../../pstack/references/principles.md#minimize-reader-load)).
