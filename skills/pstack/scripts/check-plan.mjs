...
// Structure check for a plan written from
// skills/pstack/playbooks/multi-phase-plan.md. It checks the plan's structure,
// not the truth of its evidence; that audit stays with the operator.
const RULE =
	"Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked.";
const LANE_DECL = /(\S+) lanes? at the PR head/;
const HARNESS = "verification harness";
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
// Pi plan semantics: the registered goal, the named execution playbook, a real
// audit cadence or an explicit no-scheduler statement, and the status message.
// No host markers: no /goal command, no origin/main reads, no installed-plugin
// or timed-tick assumptions.
const PROGRAM_MARKERS = ["register the program goal", "execution playbook", "audit cadence", "status message"];
...
for (const pr of prSections) {
	...
	if (live) {
		const laneDecl = live.rest.match(LANE_DECL);
		const token = laneDecl ? laneDecl[1] : "";
		const declared = /^\d+$/.test(token) ? Number(token) : 0;
		if (!token || !/\d/.test(token)) fail(live.n, `${pr.title}: Verify, live does not declare how many lanes run at the PR head`);
		else if (declared < 1) fail(live.n, `${pr.title}: Verify, live declares ${token} lanes, a positive whole count is required`);
		if (!live.rest.includes(HARNESS)) fail(live.n, `${pr.title}: Verify, live does not name the project verification harness`);
		...
		if (numbers.length !== declared || !numbers.every((n, i) => n === i + 1)) fail(live.n, `${pr.title}: lanes are [${numbers.join(",")}], expected 1 to ${declared}`);
		for (const lane of lanes) {
			if (!lane.m) fail(lane.n, `${pr.title}: live box is not a lane`);
			else if (!/Save `[^`]+`/.test(lane.text)) fail(lane.n, `${pr.title}: lane ${lane.m[1]} names no artifact to save`);
