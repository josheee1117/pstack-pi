...
4. **Run N candidates in parallel** on different models per `pstack-arena` phase B, each in its own sanitized directory, with the same prompt. Read [`../references/execution.md`](../references/execution.md) first: the candidates and the judge each need their own context, and an independent context is still independent when only one model is available. If the environment cannot run sessions in parallel, run them sequentially and say so in the report.
