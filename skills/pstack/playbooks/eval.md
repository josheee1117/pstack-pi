...
4. **Run N candidates in parallel** on different models per `pstack-arena` phase B, each in its own sanitized directory, with the same prompt. If the environment cannot run sessions in parallel, run them sequentially and say so in the report.
