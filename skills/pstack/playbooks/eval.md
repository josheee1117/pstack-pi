...
4. **Run N candidates** per `pstack-arena` phase B, preferring different available models, each in its own sanitized directory, with the same prompt. Read [`../references/execution.md`](../references/execution.md) first: the candidates and the judge each need their own fresh context. A single model is fine for every candidate, because independence comes from the fresh context and the blind read, not from the model name. If the environment cannot run sessions in parallel, run them sequentially, still one fresh context each, and say so in the report.
5. **Run one blinded judge** in a fresh context that did not produce the candidates, per `pstack-arena` phase C. Prefer a different available model family, but do not mistake model diversity for context independence. The judge sees outputs by sanitized label plus the rubric, never a model name. If no independent judge can run, follow the shared policy's [missing-backend rule](../references/execution.md#when-a-backend-is-missing).
