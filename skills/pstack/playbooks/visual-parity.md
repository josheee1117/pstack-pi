...
3. **Migrate one component at a time.** Parallelize across worktrees with one owner per component (`references/principles.md#separate-before-serializing-shared-state`). Shared primitives migrate first as a blocking phase.
...
5. Finish per `opening-a-pr.md` per component or per safe batch, when the user authorized a PR.
