...
5. **Stage the commits so the failing repro lands before the fix** in the history. See `pstack-tdd` for the failing-test-first cadence when the bug has a cheap local test path. Skip it when the test would be expensive, integration-heavy, or unclear. This is [sequence-verifiable-units](../references/principles.md#sequence-verifiable-units), the failing test first and the fix on top.
6. **Finish per [opening-a-pr](opening-a-pr.md)** when the user authorized a PR. Otherwise stop at a verified fix on your branch, committed and reported.
