...
3. **Each iteration makes the smallest change the evidence justifies**, verifies it against the predicate, commits if it advanced, and discards changes that did not help. Belt-and-suspenders that "might help" gets reverted, not left to ride. Sequence the work via `references/principles.md#sequence-verifiable-units`: verify each unit before the next instead of batching checks at the end.
