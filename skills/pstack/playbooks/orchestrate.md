...
Read [`../references/execution.md`](../references/execution.md) first. Independent sessions are the unit of work here: under the package default that means long-lived agent sessions managed by Herdr, each with its own working directory and its own git worktree when it writes, coordinating through pi-intercom when that channel exists and through files when it does not. See the entry skill's Execution environment section and the project's `AGENTS.md` for the mapping. **A program whose workers cannot get independent sessions is not a smaller program, it is a different one**, so the missing capacity stops it rather than being absorbed into the coordinator. Follow [When a backend is missing](../references/execution.md#when-a-backend-is-missing): say the capacity is missing, and let the operator pick a verified alternative backend or explicitly reduce the program's scope. The [autopilot-stack](autopilot-stack.md) playbook is a candidate to offer at that point, not a fallback to take unilaterally.
