...
Survey the window for recurring patterns. For a large corpus, split it into two to four slices and run one fresh-context reader per slice (`references/principles.md#guard-the-context-window`). Each returns a short structured list of patterns with evidence pointers. Default signals worth hunting:
